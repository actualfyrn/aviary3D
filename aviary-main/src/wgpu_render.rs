...
// Above is a more manual way of mapping out the Vertex attributes
...
impl State {
    async fn new(window: Arc<Window>) -> anyhow::Result<State> {
        ...
        let diffuse_texture = texture::Texture::from_file
            (&device, &queue, "assets/textures/example.dds", "example.dds").unwrap();
        ...
        // Below no longer used, only used if doing unindexed drawing
        ...
        let render_pipeline_layout =
            ...
            bind_group_layouts: &[
                Some(&texture_bind_group_layout).unwrap(),
                Some(&camera_bind_group_layout).unwrap()
            ],
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        // Extra brackets to drop render_pass when done
        {
            ...
            // Non-indexed draw of vertices, no longer used
